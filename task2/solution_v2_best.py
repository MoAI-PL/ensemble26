...
v2 PRECISION — scored 0.3138 avg_chrf on public (Qwen: 0.595, Mellum: 0.246, Codestral: 0.101)

Strategy: compact, import-focused context that fits within Mellum's 8K window
so all 3 models see the same high-quality content.
"""
...
PRECISION_CHARS = int(7_500 * CHARS_PER_TOKEN)
...
def parse_args():
    p = argparse.ArgumentParser(description="Context collection pipeline v2 precision")
    ...
    p.add_argument("--max-context-chars", type=int, default=PRECISION_CHARS,
                    help="Max context characters (proxy for tokens)")
    p.add_argument("--repos-stage", type=str, default=None,
                    help="Stage for repo dir (default: same as --stage). Use 'public' when stage=test to reuse public repos.")
    return p.parse_args()
...
def process_datapoint(
    ...
    # --- AST Pass 2: Symbol usage + type hints ---
    used_symbols = extract_used_symbols(prefix)
    if suffix:
        used_symbols |= extract_used_symbols(suffix)
    type_hints = extract_type_hints(prefix)
    needed_symbols = used_symbols | type_hints

    # --- AST Pass 3: Targeted extraction from imported files ---
    ...
    for imp_path in imported_paths:
        ...
        is_init = imp_path.endswith("__init__.py")
        ...
        if used_chars + entry_len > max_chars:
            ...
            if remaining_budget > 200:
                ...
                if len(skel) + len(FILE_SEP) + len(imp_path) + 1 < remaining_budget:
                    context_entries.append((imp_path, skel))
                    used_chars += len(FILE_SEP) + len(imp_path) + 1 + len(skel)
            break

        context_entries.append((imp_path, snippets))
        used_chars += entry_len

    # --- Type hint resolution ---
    type_names_to_find = type_hints - {s for s in used_symbols if len(s) <= 2}
    if type_names_to_find and used_chars < max_chars:
        for path, content in candidate_files.items():
            if path in imported_set or used_chars >= max_chars:
                break
            for tname in type_names_to_find:
                if f"class {tname}" in content:
                    skeleton = skeletonize(content)
                    skeleton = strip_docstrings(skeleton)
                    entry_len = len(FILE_SEP) + len(path) + 1 + len(skeleton)
                    if used_chars + entry_len <= max_chars:
                        context_entries.append((path, skeleton))
                        imported_set.add(path)
                        used_chars += entry_len
                    break

    # --- BM25: only top few, only skeletonized, only if budget remains ---
    if used_chars < max_chars * 0.8 and args.bm25_top_k > 0:
        remaining_files = {p: c for p, c in candidate_files.items()
                          if p not in imported_set and not p.endswith("__init__.py")}
        ...
        if remaining_files:
            ...
            if corpus:
                ...
                for score_val, path in ranked[:args.bm25_top_k]:
                    if score_val <= 0 or used_chars >= max_chars:
                        break
    ...
    # --- Compose ---
    context = "".join(f"{FILE_SEP}{path}\n{content}" for path, content in context_entries)
...
def main():
    ...
    repos_stage = args.repos_stage if args.repos_stage is not None else stage
        ...
        with jsonlines.open(output_file, "w") as writer:
            for datapoint in tqdm(reader, total=total, desc="Processing"):
                ...
                root_dir = os.path.join(
                    "data",
                    f"repositories-{language}-{repos_stage}",
